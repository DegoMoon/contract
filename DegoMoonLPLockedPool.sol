...
/*
    DegoMoon Based on Dego token powered by safemoon,the hope of dego！no ico , no presale,
...
contract DegoMoonLPLockedPool is Context,Ownable {
    ...
    //unlock time
    uint256 public _unLockBlock;
    //10 years,every 5 seconds 1 block
    uint256 constant _totalLockedBlock = 63072000;
    
    constructor () public {
        _unLockBlock = block.number.add(_totalLockedBlock);
    }
    
    function setLPTokenAddress(address token) public onlyOwner{
    ...
    function addLockedBlock() public onlyOwner {
        _unLockBlock = _unLockBlock.add(_totalLockedBlock);
    }
    
    function withdrawAllLP() public onlyOwner {
        uint256 amount = _LPtoken.balanceOf(address(this));
        require(block.number >= _unLockBlock,"The current block number less than unlock block number!");
